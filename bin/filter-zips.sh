...
This will filter a list of file names or grep output looking for compressed, archive file extensions.

path    File names to process. If omitted then standard input will be scanned.
-v      Filter out the archive files and show all other files.
...
See also egrep filter-text.sh, filter-docs.sh, filter-videos.sh, filter-fonts.sh, filter-scripts.sh, filter-web.sh, filter-css.sh, filter-images.sh, filter-sounds.sh, classify.sh
