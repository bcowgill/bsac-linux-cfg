...
perl -ne '
  ...
  if ($id =~ m{\A([rl][sd]q|sp|nbsp|endash)\z}xms || $id =~ m{_confluence|languagePicker\.label\.}xms)
  {
    $translate = $language;
    $translate =~ s{/\d+/}{/TODO/}xms;
    $notes = "LEAVE AS IS";
  ...
  sub warning {
    my ($message) = @_;
    warn "$ENV{ERRFILE} line $.: $message\n";
  }
  sub check {
    my ($changed, $type, $name, $id, $qq, $to) = @_;
    if ($id =~ m{$qq}xms)
    {
      ++$changed;
      my $found = $id =~ tr[$qq][$qq];
      warning("COUNT: $found");
      if ($found % 2)
      {
        warning("Odd number of $type quote $qq found in $name, change to $to; cannot correct automatically: $id")
      }
      else
      {
        warning("$type quote $qq found in $name, change to $to: $id");
      }
    }
    return $changed;
  }
  sub fix {
    my ($id, $language) = @_;
    my $changed = 0;
    $changed = check($changed, "Double", "id", $id, $qq, join("", @qqr));
    $changed = check($changed, "Double", "message", $language, $qq, join("", @qqr));
    unless ($SQOK) {
      $changed = check($changed, "Single", "id", $id, $q, join("", @qr));
      $changed = check($changed, "Single", "message", $language, $q, join("", @qr));
    }
    if ($changed) {
      ...
      print STDERR qq{CORRECTED:  "$id": "$language",\n};
    }
    return ($id, $language);
  }
  sub fix_quote {
