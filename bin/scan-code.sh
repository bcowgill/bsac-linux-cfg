...
\b(maxcomplexity|maxstatements|maxlen|latedef|strict|unused)\s*:|\b(eqeqeq\s*:\s*false)|\bconsole\.(log|info|warn|error|dir|time|timeEnd|trace|assert)|\balert\(|\.(skip|only)\(|\(\s*[a-zA-Z_]\w?\s*\)|\bvar\s+([a-zA-Z]\w?|\w\w)\b' \
{} \; \
\)
