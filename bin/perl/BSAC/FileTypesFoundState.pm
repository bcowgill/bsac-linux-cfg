...
BEGIN {
	...
	unless ($BSAC::FileTypesFoundState::CLASS_FILENAME) {
		$BSAC::FileTypesFoundState::CLASS_FILENAME = Carp::shortmess();
		$BSAC::FileTypesFoundState::CLASS_FILENAME =~ s{\A \s+ at \s+ (.+) \s+ line \s+ \d+ \. \s* \z}{$1}xms;
	}
	if (defined($BSAC::FileTypesFoundState::CLASS_FILENAME) && -e $BSAC::FileTypesFoundState::CLASS_FILENAME) {
