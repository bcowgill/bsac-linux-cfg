 ...
 */
export function getDumpableObject(thing: Record<string, unknown>): Record<string, unknown> {
...
function strV(what?: any): string | undefined {
	...
	if (what instanceof Date) {
		...
		whatStr = `${getTypeName(what) || typeof what}: ${fixFunction(what.toString())}`;
	}
	return whatStr;
} // strV()
...
expectToBe(fixFunction('function(){}'), 'function () { }', 'fixFunction(anon function) should adjust spacing of functions');
...
expectToBe(fixFunction('function blob (){}'), 'function blob () { }', 'fixFunction(named function spaced) should adjust spacing of named functions');

expectToBe(fixFunction('()=>{}'), '() => { }', 'fixFunction(arrow function) should adjust spacing of arrow');
expectToBe(fixFunction('  ()=>{}'), '  ()=>{}', 'fixFunction(lead spaces arrow) should leave alone');
expectToBe(fixFunction('(x)=>null'), '(x) => null', 'fixFunction(arrow param) should adjust spacing of arrow function with parameter');
expectToBe(fixFunction('(x)=>{\n\n}'), '(x) => {\n\n}', 'fixFunction(arrow param newlines) should adjust spacing of arrow function with parameter and newlines');
expectToBe(fixFunction('()=>{return 42}'), '() => { return 42 }', 'fixFunction(arrow function body) should adjust spacing of arrow functions with body');
expectToBe(fixFunction('()=>{ return 42; }'), '() => { return 42; }', 'fixFunction(arrow function body spaced) should adjust spacing of arrow functions with body spaced');
...
expectToEqual(window.location.searchParams, expectSearchParamsNone, 'should set window searchParams equal');
window
	.location
...
expectToBe(window.location.search, '?param=34&that=this', 'should set window search params stringify');
expectToBe(window.location.hash, '#hashpath', 'should set window hash');
expectToEqual(window.location.searchParams, expectSearchParams, 'should set window searchParams object');
window
	.location
	.replace('fil\e:/path');
window
...
expectToBe(getTypeName(function named3() : void {}), 'Function ', 'getTypeName(function named) should return Function');
...
expectToBe(showThing(function () : void {}, 26), expectNoop, 'showThing(anon function,26) should have no ellipsis');
expectToBe(showThing(function named5() : void {}, 20), expectNamed5, 'showThing(named function) should have no ellipsis');
expectToBe(showThing(noop, 19), expectNoop2, 'showThing(function name) should have no ellipsis');
...
expectToBe(showThing(() => null, arrowEllipsisLen), expectArrowNullEllipsis, 'showThing(arrow function,11/8) should have ellipsis');
expectToBe(showThing(function () : string { return 'S'; }, anonEllipsisLen), expectAnonEllipsis, 'showThing(anon function,13) should have ellipsis');
expectToBe(showThing(function name6() : void {}, named6EllipsisLen), expectNamed6Ellipsis, 'showThing(name6 function,8/7) should have ellipsis');
expectToBe(showThing(noop, noopEllipsisLen), expectNoopEllipsis, 'showThing(function name,8/6) should have ellipsis');
