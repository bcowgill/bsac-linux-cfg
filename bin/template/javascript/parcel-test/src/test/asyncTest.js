...
export function expectPromiseReject(description, asyncDone, promise, fnTest) {
  ...
  promise
    .then(
      ...
      (response) => {
        try {
          expect(response).toBe('Promise should have rejected with reason, but resolved instead');
        } catch (error) {
          warn(`● ${description}\n${error.toString()}`);
        }
        return response;
...
export function expectPromiseResolves(description, asyncDone, promise, fnTest) {
  ...
  promise
    ...
    .catch(
      ...
      (reason) => {
        try {
          expect(reason).toBe('Promise should have resolved with a response, but rejected instead');
        } catch (error) {
          warn(`● ${description}\n${error.toString()}`);
        }
      });
