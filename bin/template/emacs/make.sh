#!/bin/bash
# make various emacs references based on key configuration
...
if [ "$1" == "launch" ]; then
	edit.sh emacs-card.pl emacs-tutorial.markup.txt emacsercises.markup.txt \
		emacs-keyref.html emacs-ergo-keyref.html emacs-tutorial.txt \
		emacsercises.txt emacs.fortune.txt
