...
This will output everything from second.json which has an identical key from first.json.
...
$cmd first.json second.json > second-shaved.json
USAGE
	exit 0;
}
...
while (my $line = <>)
{
	debug("$fileName firstFile: $firstFile first: $first second: $second common: $common something: $something First: @{[scalar(keys(%First))]}\n");
	...
	{
		# output from second file if identical key from first file.
		++$second;
		if ($Keys{$key})
		{
		...
		if ($First{$key})
		{
			debug("common k: $key v: $value 1: $First{$key}\n");
			++$common;
			print ",\n" if $something;
			print qq{  "$key": $value};
			$something = 1;
