...
PROGRAM=../../filter-json-commas.sh
CMD=`basename $PROGRAM`
SAMPLE=in/SAMPLE.json
DEBUG=
SKIP=0
HEAD=3
...
echo TEST $CMD successful operation inplace
TEST=success
if [ 0 == "$SKIP" ]; then
	ERR=0
	OUT=out/$TEST.out
	...
	ARGS="$DEBUG"
	cp $SAMPLE $OUT
	INPLACE=-i $PROGRAM $ARGS $OUT || assertCommandSuccess $? "$PROGRAM $ARGS"
	filter "$OUT"
	assertFilesEqual "$OUT" "$BASE" "$TEST"
else
	echo SKIP $TEST "$SKIP"
