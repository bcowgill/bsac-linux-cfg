...
echo TEST $CMD scanning for elements
TEST=scan-elements
if [ 0 == "$SKIP" ]; then
	ERR=0
	OUT=out/$TEST.out
...
echo TEST $CMD inplace edit of elements
TEST=edit-elements
if [ 0 == "$SKIP" ]; then
	ERR=0
	OUT=out/$TEST.out
...
echo TEST $CMD inplace edit of a template
TEST=edit-template
...
echo TEST $CMD scanning for complex elements
TEST=scan-complex
