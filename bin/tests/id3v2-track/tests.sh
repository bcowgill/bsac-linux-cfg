...
PROGRAM=../../id3v2-track.sh
CMD=`basename $PROGRAM`
SAMPLE=../get-meta/in/TestId3Tagv2basic.mp3
SAMPLE1=../../sounds/TestId3/*.mp3
DEBUG=
SKIP=0
HEAD=3
...
echo TEST $CMD successful operation listing
TEST=success-list
if [ 0 == "$SKIP" ]; then
	ERR=0
	OUT=out/$TEST.out
	BASE=base/$TEST.base
	ARGS="$DEBUG"
	$PROGRAM $ARGS $SAMPLE1 > $OUT 2>&1 || assertCommandSuccess $? "$PROGRAM $ARGS"
	filter "$OUT"
	assertFilesEqual "$OUT" "$BASE" "$TEST"
else
	echo SKIP $TEST "$SKIP"
